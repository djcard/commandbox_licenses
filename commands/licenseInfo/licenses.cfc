...
component {
	 ...
	 **/
	void function run( string folder = getCwd(), string exportTo="screen", filename="licenseData" ){
	...
	void function outPutToScreen( displayData ){
		...
		displayData.each((item)=>{
			print.line( makeIndent(item.indents) & "|-" & item.name & " (" & item.version & ") : " & item.licenseData.toList( "," ) );
		});
	}
	...
	function createUnique(displayData, filename){
		...
		var filteredData = displayData.each((item)=>{
			...
			filtered[item.name][item.version] = filtered[item.name].keyExists(item.version) ? filtered[item.name][item.version] : [];
		...
		var output = "name,version,license,indent#chr(10)#";
		filtered.keyArray().sort("textNoCase").each((packageName)=>{
			
			var data = filtered[packageName];
			data.keyArray().sort("textNoCase").each((versionName)=>{
